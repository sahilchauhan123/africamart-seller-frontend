...
const EditProfile: React.FC<Props> = ({ onBack, onSave }) => {
  return (
    <div className="bg-white dark:bg-slate-900 font-sans text-slate-900 dark:text-slate-100 antialiased h-full flex flex-col overflow-hidden transition-colors duration-200">
      {/* Mobile View */}
      <div className="lg:hidden h-full flex flex-col overflow-hidden">
        ...
        <main className="flex-1 w-full max-w-md mx-auto px-6 py-8 overflow-y-auto no-scrollbar">
          ...
          <div className="mb-10 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
          ...
          <div className="flex flex-col items-center mb-10">
            <div className="relative">
              <div className="w-32 h-32 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-4xl font-black text-primary border-4 border-white dark:border-slate-700 shadow-2xl overflow-hidden">
                {MOCK_USER.businessName.charAt(0)}
              </div>
              <button className="absolute bottom-1 right-1 bg-primary text-white p-2.5 rounded-full border-4 border-white dark:border-slate-700 shadow-md active:scale-90 transition-transform">
                <span className="material-icons-round text-lg leading-none">photo_camera</span>
              </button>
            </div>
            <h2 className="mt-5 text-2xl font-black text-slate-900 dark:text-white tracking-tight">Hi, {MOCK_USER.name.split(' ')[0]}</h2>
          ...
          <form className="space-y-6 pb-24" onSubmit={(e) => { e.preventDefault(); onSave(); }}>
            ...
            <div className="pt-6">
              <button
                type="submit"
                className="w-full bg-primary hover:bg-blue-800 text-white font-black py-5 rounded-[20px] shadow-xl shadow-blue-100 dark:shadow-none transition-all active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-blue-50 tracking-wide text-sm uppercase"
              >
                Update Profile
              </button>
            </div>
        ...
        <div className="max-w-[1536px] mx-auto">
          ...
          <div className="flex items-center gap-8 mb-16">
            <div className="relative flex-shrink-0">
              <div className="w-24 h-24 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <span className="text-3xl font-bold text-slate-400">{MOCK_USER.businessName.charAt(0)}</span>
              </div>
              <button className="absolute -bottom-1 -right-1 w-8 h-8 bg-primary text-white rounded-full flex items-center justify-center border-2 border-background-light dark:border-background-dark hover:scale-110 transition-transform shadow-md">
                <span className="material-symbols-outlined text-sm">add_a_photo</span>
              </button>
            </div>
            <div>
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Hi, {MOCK_USER.name}</h2>
              <p className="text-slate-500 dark:text-slate-400 mt-1 text-lg">Manage your business identity and verification details.</p>
            </div>
          </div>
          ...
          <form className="space-y-16 pb-20" onSubmit={(e) => { e.preventDefault(); onSave(); }}>
            <div>
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-semibold text-slate-900 dark:text-white">General Information</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                <div className="space-y-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider">Business Name</label>
                  <input
                    className="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white font-medium placeholder-slate-400 focus:bg-white dark:focus:bg-slate-900 focus:border-primary focus:ring-0 transition-all outline-none"
                    placeholder="e.g. Acme Solutions"
                    type="text"
                    defaultValue={MOCK_USER.businessName}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white font-medium placeholder-slate-400 focus:bg-white dark:focus:bg-slate-900 focus:border-primary focus:ring-0 transition-all outline-none appearance-none cursor-pointer">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <textarea
                    className="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white font-medium placeholder-slate-400 focus:bg-white dark:focus:bg-slate-900 focus:border-primary focus:ring-0 transition-all outline-none resize-none"
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Tax & Verification</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                <div className="space-y-2">
            ...
            <div className="pt-8 flex items-center justify-between border-t border-slate-200 dark:border-slate-700">
