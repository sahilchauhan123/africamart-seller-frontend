    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-[85%] max-w-[300px] bg-white z-[70] shadow-2xl flex flex-col transition-transform duration-300 lg:translate-x-0 lg:shadow-none lg:border-r lg:z-40 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="bg-primary pt-12 pb-6 px-5 text-white relative">
          <button className="absolute top-4 right-4 text-white z-10 p-1 bg-white/20 rounded-full hover:bg-white/30 transition lg:hidden" onClick={onClose}>
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div className="flex-grow">
              ...
              <button onClick={() => { onNavigate(View.PROFILE); if (window.innerWidth < 1024) onClose(); }} className="flex items-center gap-1 text-blue-100 text-xs hover:text-white transition-colors mt-0.5">
        ...
        <nav className="flex-1 overflow-y-auto no-scrollbar">
          <div className="py-4">
            <button onClick={() => { onNavigate(View.DASHBOARD); if (window.innerWidth < 1024) onClose(); }} className="w-full flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50 transition-colors text-gray-600">
              <span className="material-icons-round">home</span>
              <span className="font-bold text-sm tracking-wide">HOME</span>
            </button>
            <button onClick={() => { onNavigate(View.PRODUCT_LIST); if (window.innerWidth < 1024) onClose(); }} className="w-full flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50 transition-colors text-gray-600">
              <span className="material-icons-round">inventory_2</span>
              <span className="font-medium text-sm">Manage Products</span>
            </button>
            <button onClick={() => { onNavigate(View.INQUIRY_LIST); if (window.innerWidth < 1024) onClose(); }} className="w-full flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50 transition-colors text-gray-600">
          ...
          <div className="pt-5 pb-2">
            ...
            <button onClick={() => { onNavigate(View.MESSAGES); if (window.innerWidth < 1024) onClose(); }} className="w-full flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50 transition-colors text-gray-600">
